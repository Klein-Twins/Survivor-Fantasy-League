...
describe('loginService.login', () => {
  ...
  const mockUserRecord: UserAttributes = {
    USER_ID: '23423',
    USER_EMAIL: mockFields.email,
    USER_PROFILE_ID: '32525-2342242-234242',
    USER_NAME: 'HELLO_TEST_WORLD',
  };
  ...
  it('should throw an error if email or password is missing', async () => {
    ...
    await expect(loginService.login(invalidFields)).rejects.toThrow('Email and password are required');
  ...
  it('should throw an error if no account is found for the provided email', async () => {
    ...
    await expect(loginService.login(mockFields)).rejects.toThrow('No account tied to the provided email');
  ...
  it('should handle errors when tokenService.generateToken throws an error', async () => {
    ...
    await expect(loginService.login(mockFields)).rejects.toThrow('Token generation failed');
