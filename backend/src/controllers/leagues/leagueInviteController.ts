...
import { CreateAndSendLeagueInviteRequestBody, CreateAndSendLeagueInviteResponse, GetLeagueInvitesForPlayerResponse, League, LeagueInvite, RespondToLeagueInviteRequestBody, RespondToLeagueInviteRequestBodyInviteResponseEnum, RespondToLeagueInviteResponse, RespondToLeagueInviteResponseData } from "../../generated-api";
...
async function getLeagueInvitesForProfileId(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
        ...
        const leagueInvites: LeagueInvite[] = await leagueMemberRepository.getLeagueInvitesForProfileId(profileId as string);
...
async function respondToLeagueInvite(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
        ...
        const responseData: RespondToLeagueInviteResponseData | undefined = league ? {
            league
        } : undefined

        const response: RespondToLeagueInviteResponse = {
            success: true,
            message: requestBody.inviteResponse === RespondToLeagueInviteRequestBodyInviteResponseEnum.ACCEPT ? "You have successfully joined the league" : "You have declined the league invite",
            statusCode: 200,
            responseData
        }
