...
async function profileSearchQuery(
  params: ProfileSearchParams
): Promise<{ foundProfiles: ProfileAndLeagueInviteStatus[]; totalCount: number }> {
  const { firstName, lastName, userName, sortBy, isAsc = 'true', page = 1, limit = 10 } = params;
...
async function getProfileByProfileId(profileId: string): Promise<Profile | null> {
  ...
  const userName = await userRepository.getUserNameByProfileId(profileAttributes.profileId);
  if (!userName) {
    return null;
  }
