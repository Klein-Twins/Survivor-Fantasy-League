...
async function createLeague(seasonId: number, name: string, profileId: string, options: CreateOptions): Promise<League | null> {
    ...
    const leagueOwner: LeagueMember | null = await leagueMemberRepository.createLeagueMember(leagueAttributes.leagueId, profileId, null, LeagueMemberRoleEnum.OWNER, InviteStatusEnum.Accepted, options);
    if (!leagueOwner) {
