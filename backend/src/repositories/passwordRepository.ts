...
const passwordRepository = {
   ...
   */
  setPasswordRecordToInactive: async (passwordRecord: PasswordAttributes, transaction?: Transaction): Promise<void> => {
    try {
      await models.Password.update(
        { active: false },
        {
      ...
      logger.error(
        `Failed to set password seq ${passwordRecord.passwordSeq} to inactive for user with user id ${passwordRecord.userId}: ${error}`
      );
      throw error;
    }
  },
  ...
  getActivePasswordForUserId: async (userId: UserAttributes['userId']): Promise<PasswordAttributes> => {
    ...
    if (userPasswords.length > 1) {
      ...
      try {
        ...
        await Promise.all(
          userPasswords.slice(1).map((pw) => passwordRepository.setPasswordRecordToInactive(pw, transaction))
        );
   ...
   * @returns A promise that resolves when the password is deactivated.
   */
  setInactivePassword: async (passwordRecord: PasswordAttributes, transaction?: Transaction): Promise<void> => {
    await models.Password.update(
      { active: false },
