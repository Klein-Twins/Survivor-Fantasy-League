...
import profileService from '../../servicesBackup/account/profileService';
...
async function searchForProfilesToInviteToLeague({
  ...
  for (const result of results) {
    ...
    const isInvited: boolean = result.inviteStatus !== InviteStatus.Pending;
    const isJoined: boolean = result.inviteStatus === InviteStatus.Accepted;
    foundProfiles.push({
      profile,
      isInvited,
      isJoined,
