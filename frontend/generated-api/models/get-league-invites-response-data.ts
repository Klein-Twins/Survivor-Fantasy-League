/* tslint:disable */
/* eslint-disable */
/**
 * Survivor Fantasy League - OpenAPI 3.0.3
 * This is the Survivor Fantasy League Spec for API's
 *
 * OpenAPI spec version: 1.0.11
 * Contact: pklein111697@gmail.com
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

import { LeagueInvite } from './league-invite';
 /**
 * 
 *
 * @export
 * @interface GetLeagueInvitesResponseData
 */
export interface GetLeagueInvitesResponseData {

    /**
     * @type {Array<LeagueInvite>}
     * @memberof GetLeagueInvitesResponseData
     */
    leagueInvites: Array<LeagueInvite>;

    /**
     * The number of league invites returned
     *
     * @type {number}
     * @memberof GetLeagueInvitesResponseData
     * @example 4
     */
    numLeagueInvites: number;
}
